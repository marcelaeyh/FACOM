...
df = pd.read_sql(q,con=eng)
df.to_csv('/Users/mac/Desktop/FACOM/1_Proyectos/5_volcan/estaciones_presion.csv')

q2 = '''
SELECT CodigoEstacion, Departamento, Municipio,Latitud,Longitud,FechaObservacion,ValorObservado
FROM presion 
WHERE FechaObservacion 
LIKE '01/15/2022%'
'''
df = pd.read_sql(q2,con=eng)
...
for i in range(len(n)):
    ...
    if len(estacion.ValorObservado) > 24:
        ...
        estacion = estacion.drop(columns = ["FechaObservacion","ValorObservado"])
        estacion = estacion.drop_duplicates()
        volcan = volcan.append(estacion)
