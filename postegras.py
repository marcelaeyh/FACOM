...
conn=engine.connect()
#sqlite
T_sqlite= 'sqlite:////home/marcelae/Desktop/FACOM/db/temperatura_2.db'
P_sqlite= 'sqlite:////home/marcelae/Desktop/FACOM/db/precipitacion_2.db'
...
temp = r"/home/marcelae/Desktop/FACOM/csv/Datos_Hidrometeorol_gicos_Crudos_-_Red_de_Estaciones_IDEAM___Temperatura.csv"
pre = r"/home/marcelae/Desktop/FACOM/csv/recipitaci_n.csv"
pres= r"/home/marcelae/Desktop/FACOM/csv/Presi_n_Atmosf_rica.csv"
coor= r"/home/marcelae/Desktop/FACOM/Estaciones/coordenadas_estaciones.csv"
#conjunto de datos
...
while tqdm(cont <= (n_p-1)):
    ...
    for index, row in tqdm(df.iterrows()):
        ...
        if row[vnCSV[3]] < 0.0:
            df[vnC[19]][index] = 1.0
        if ((row[vnCSV[3]] > 1.5) and (row[vnCSV[3]] <= 2.0 )) :
             df[vnC[19]][index] = 1.0
        if row[vnCSV[3]] > 2.0:
            df[vnC[19]][index] = 2.0
    
    #Relacion con la llave foranea
    p=0
    for i in tqdm(range(n_df)):
        fecha_antioquia=df[vnCSV[2]][i]
        for j in range(p,n_mo):
            fecha_tabla=mo[vnBD[20]][j]
            if (fecha_antioquia==fecha_tabla):
                p=j
                df[vnC[20]][i]= int(mo[vnBD[21]][j])
                break
    ...
    V=pd.DataFrame(V)
    vnBD[25]
    V.columns=[vnBD[22], vnBD[21],vnBD[23],vnBD[24],vnBD[25]]
...
while tqdm(cont <= (n_t-1)):
    ...
    for index, row in tqdm(df.iterrows()):
        ...
        if row[vnCSV[3]] < -10.0:
            df[vnC[19]][index] = 2.0 
        if ((row[vnCSV[3]] > 40.0) and (row[vnCSV[3]] <= 50.0 )) :
             df[vnC[19]][index] = 1.0
        if row[vnCSV[3]] > 60.0:
            df[vnC[19]][index] = 2.0
    print("Ingresa a relacionar-",dx)
    print("#------#-------#")
...
#6.11.2 PRESION
lpe =pd.read_csv(pres,usecols=[0])
n_pe=len(lpe)

step=math.ceil(n_pe*0.0)  #el número es el porcentaje que se va a tomar "dx"
cont=0 # el contador inicia desde 0, pero si es necesario se pue asignar uno diferente
dx=0
while tqdm(cont <= (n_pe-1)):
    start= time.time()
    #La siguiente fila de codigo lo que carga es el dx, se toma una porción y solo se carga
    #el porcentaje que se desea cargar que inicialmente se asigno en cada paso. 
    df=pd.read_csv(pres,nrows=int(step),skiprows=range(1,int(cont)),usecols=[0,2,3])
    print("#------#-------#")
    print("contador",cont,"paso=",dx)
    print("#------#-------#")
